...
def _classify_fingertips_shadow(robot):
    """Heuristically classify Shadow Hand fingertips.
    Return (thumb_index, other_indices).

    Shadow URDF typically uses prefixes: th* (thumb), ff* (index), mf*, rf*, lf*.
    We'll prefer names that contain 'th' or 'thumb'. Fallback to first matching.
    """
    tips = _get_fingertip_links(robot)
    names = [lnk.get_name().lower() for lnk in tips]
    thumb_idx = None
    candidates = [i for i, n in enumerate(names) if ("thtip" in n or n.startswith("th") or "thumb" in n)]
    if candidates:
        thumb_idx = candidates[0]
    else:
        # fallback: try the one with 'th' anywhere
        for i, n in enumerate(names):
            if "th" in n:
                thumb_idx = i
                break
    if thumb_idx is None:
        # as a last resort, choose the first or last link depending on naming order
...
def optimize_shadow_to_fixed_object(
    ...
    """Optimize Shadow Hand qpos to contact object using nearest points, with smooth transition.

    Mirror of Allegro optimizer but targets viewer.robots[robot_index] (default 1 for Shadow).
    """
    ...
    for t in range(Tn):
        ...
        for _ in range(iters_per_frame):
            ...
            # Encourage thumb-other opposition for better wrap
            try:
                thumb_idx, other_idxs = _classify_fingertips_shadow(robot)
